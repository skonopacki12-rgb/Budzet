  ...
  return (
    <div className="flex flex-col gap-5 pt-2">
      ...
      <img
        ...
        className="max-h-64 w-full rounded-2xl border border-neutral-200 object-contain"
      />
      ...
      {(receipt.status === "queued" || receipt.status === "ocr" || receipt.status === "ai") && (
        <p className="text-sm text-neutral-500">Analizowanie w toku…</p>
      )}
            ...
            {confirmedItems.map((item) => {
              ...
              return (
                <li key={item.id} className="flex items-center justify-between py-2 text-sm">
                  <div>
                    <p className="text-neutral-900">{item.rawName}</p>
                    <p className="text-xs text-neutral-400">{category?.name ?? "Bez kategorii"}</p>
                  </div>
                  <span className="font-medium text-neutral-900">{formatPln(item.totalPrice)}</span>
                </li>
              );
            })}
          </ul>
      ...
      {unconfirmed.length > 0 && (
        <section>
          <h2 className="mb-2 text-sm font-medium text-neutral-700">Do potwierdzenia</h2>
          <ReceiptItemsForm
            receiptId={id}
            items={unconfirmed}
      ...
      <form action={deleteReceipt} className="border-t border-neutral-100 pt-4">
        ...
        <ConfirmButton
          ...
          className="rounded-lg border border-red-200 px-3 py-2.5 text-sm font-medium text-red-600"
        >
          Usuń paragon
        </ConfirmButton>
      </form>
