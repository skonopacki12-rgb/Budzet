  ...
  return (
    <div className="flex flex-col gap-6 pt-2">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-neutral-900">Paragony</h1>
        <Link href="/paragony/nowy" className="rounded-lg bg-neutral-900 px-3 py-1.5 text-sm font-medium text-white">
          Dodaj paragon
        </Link>
      </div>

      {deleted && (
        <div className="rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-800">Paragon usunięty.</div>
      )}
            ...
            <li key={receipt.id}>
              ...
              >
                ...
                {receipt.totalAmount != null && (
                  <span className="font-medium text-neutral-900">{formatPln(receipt.totalAmount)}</span>
                )}
              </Link>
            </li>
          ))}
