  ...
  return (
    <div className="flex flex-col gap-6 pt-2">
      <div className="flex flex-col gap-1.5">
        <h1 className="text-xl font-semibold text-neutral-900 dark:text-neutral-100">Cele oszczędnościowe</h1>
      ...
      <section>
        {goals.length === 0 ? (
          <p className="text-sm text-neutral-400 dark:text-neutral-500">Brak celów — dodaj pierwszy poniżej.</p>
        ) : (
          <ul className="flex flex-col gap-3">
            {goals.map((goal) => {
              ...
              return (
                <li
                  key={goal.id}
                  className={`rounded-2xl border p-4 ${goal.achieved ? "border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950" : "border-neutral-200 dark:border-neutral-800"}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      {goal.targetDate && (
                        <p className={`text-xs ${overdue ? "font-medium text-red-600 dark:text-red-400" : "text-neutral-500 dark:text-neutral-400"}`}>
                  ...
                  <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-neutral-100 dark:bg-neutral-800">
                    <div
                      className={`h-full rounded-full ${goal.achieved ? "bg-emerald-500" : "bg-neutral-900 dark:bg-neutral-100"}`}
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    <form action={addContribution} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        inputMode="decimal"
                        placeholder="np. 100"
                        className="w-24 rounded-lg border border-neutral-300 dark:border-neutral-700 px-2 py-1.5 text-sm outline-none focus:border-neutral-900 dark:focus:border-neutral-100"
                      />
                      <button
                        type="submit"
      ...
      <section className="flex flex-col gap-3 border-t border-neutral-100 dark:border-neutral-800 pt-6 md:max-w-md">
        <h2 className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Dodaj cel</h2>
